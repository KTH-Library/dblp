...
# happens especially from shared CI runner IPs), and can hang rather
# than returning an error promptly, so calls below are bounded by a
# wall-clock timeout and wrapped to degrade gracefully instead of
# failing (or stalling) the vignette build
safe_content <- function(expr) {
  tryCatch({
...
# (this happens especially from shared CI runner IPs), and can hang
# rather than returning an error promptly, so this example is bounded
# by a wall-clock timeout and wrapped to degrade gracefully instead of
# failing (or stalling) the vignette build
link <- tryCatch({
  setTimeLimit(elapsed = 15, transient = TRUE)
  on.exit(setTimeLimit(elapsed = Inf, transient = FALSE))
